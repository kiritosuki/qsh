...
func PrintConfigErrorMessage(err error) {
	...
	messageString := fmt.Sprintf(
		"---\n"+
			"# Options:\n\n"+
			"1. Run `%s` to load the automatic backup.\n"+
...
func RevertAppConfigToBackup() error {
	...
	// 若存在配置文件 先删除
	if err := os.Remove(fullFilePath); !os.IsNotExist(err) && err != nil {
		return err
	}
	config, err := loadExistingConfig(BackupConfigFilePath)
	if err != nil {
		return err
	}
...
func mainMenu(appConfig AppConfig) list.Model {
	items := []menuItem{
		...
		{
			title:     "Contribute",
			selectCmd: openBrowser("https://github.com/kiritosuki/qsh"),
		},
		{
			title:     "Quit",
			data:      "esc",
	...
	return defaultList("qzh Config", items)
}
...
func defaultModelSelectMenu(appConfig AppConfig) list.Model {
	...
	for _, model := range appConfig.Models {
		...
		modelItems = append(modelItems, menuItem{
			title:     m.ModelName,
			selectCmd: tea.Sequence(setDefaultModel(m.ModelName), back()),
		})
	}
...
func openEditor() tea.Cmd {
	...
	c := exec.Command(editor, fullFilePath)
...
func configureModelsMenu(appConfig AppConfig) list.Model {
	...
	for _, model := range appConfig.Models {
		...
		modelItems = append(modelItems, menuItem{
			title:     m.ModelName,
			selectCmd: setMenu(modelDetailsMenu(m)),
		})
	}
	modelItems = append(modelItems, menuItem{
		title:     "Add Model",
